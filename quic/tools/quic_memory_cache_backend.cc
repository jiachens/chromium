...
void QuicMemoryCacheBackend::ResourceFile::Read(const QuicString& cache_directory) {
  ReadFileContents(file_name_, &file_contents_);

  spdy_headers_[":status"] = "200";
  QuicString origin_url(file_name_);
  size_t start_url = origin_url.find_last_of('/');
  QuicString directory = origin_url.substr(0, start_url);
  QuicString file = origin_url.substr(start_url);
  size_t second_start = directory.find_last_of('/');
  QuicString folder = directory.substr(second_start);
  spdy_headers_["x-original-url"] = "https://beirut.eecs.umich.edu" + folder + file;
...
bool QuicMemoryCacheBackend::InitializeBackend(
  ...
  for (const auto& filename : files) {
    ...
    resource_file->Read(cache_directory);
